...
def run_dict(run: Run) -> dict:
    ...
    return {name: getattr(run, name) for name in [
        ...
        "quality_label": (run.checks or {}).get("quality_level", "scaffold_ready") if run.status == "READY" else None,
    }
    ...
    def save_clarification(
        ...
        *,
        ...
        with self.db.session() as session:
            ...
            p.messages = messages
            # Store provenance with the JSON analysis, so no destructive schema migration is needed.
            p.clarification = {**item, "conversation_revision": conversation_revision(messages)}
            return self.project_dict(p)
    ...
    def create_run(self, owner: str, project_id: str, value: RunInput) -> dict:
        ...
        try:
            with self.db.session() as session:
                ...
                current_revision = conversation_revision(p.messages)
                if value.expected_revision and value.expected_revision != current_revision:
