...
function changeProvider() { form.base_url = meta.value?.base_url || meta.value?.default_base_url || ""; form.model = ""; form.api_key = ""; form.api_version = ""; form.litellm_params = {timeout: 90, num_retries: 0, drop_params: true}; form.credentials = {}; discovered.value = []; ++discoveryEpoch; if(form.provider === "chatgpt") form.base_url = ""; }
async function discover() {
  const epoch = ++discoveryEpoch; discovering.value = true;
...
function stopAuth() { ++authEpoch; if(timer) clearTimeout(timer); authBusy.value = false; }
async function pollAuth(id: string, epoch = authEpoch) {
  if (epoch !== authEpoch || !authVisible.value || authProfile.value?.id !== id) return;
...
<template>
  <div ref="panel" v-loading="busy" class="rnd-embedded" :style="{height: panelHeight}">
    ...
    <el-drawer v-model="editor" :title="editing ? '编辑 LiteLLM 模型配置' : '添加 LiteLLM 模型'" size="min(680px, 100vw)" append-to-body class="rnd-form"><el-form label-position="top"><div class="rnd-editor-grid"><el-form-item class="wide" label="配置名称 / model_name"><el-input v-model="form.name" maxlength="80" placeholder="例如：主规划模型 / 本地测试" /></el-form-item><el-form-item v-if="!editing" label="供应商分类"><el-select v-model="category" clearable placeholder="全部分类"><el-option v-for="c in catalog.categories" :key="c" :label="c" :value="c" /></el-select></el-form-item><el-form-item label="供应商" :class="{wide: !!editing}"><el-select data-testid="provider-select" v-model="form.provider" filterable :disabled="!!editing" @change="changeProvider"><el-option v-for="p in matches" :key="p.id" :value="p.id" :label="p.name" /></el-select></el-form-item><div class="wide rnd-note" style="margin-bottom:12px">{{ meta?.description }} <el-link v-if="meta?.docs" type="primary" @click="openService(meta.docs)">接入文档</el-link></div>
      ...
      <el-form-item v-if="!['chatgpt','bedrock','vertex_ai'].includes(form.provider)" class="wide" :label="editing?.has_api_key ? 'API Key（留空保留原值）' : 'API Key'"><el-input data-testid="api-key" v-model="form.api_key" type="password" show-password autocomplete="new-password" /></el-form-item>
