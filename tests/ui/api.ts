...
const api={
 ...
 async saveIntegration(id:string,revision:number,values:any){
   if(revision!==(configurations[id]?.revision || 0)) throw new Error("stale fixture revision");
   (window as any).__configCalls.push({id,action:"save",revision});
   configurations[id]={revision:revision+1,value:values.fixture}; return this.integrationConfig(id);
 },
 async resetIntegration(id:string,revision:number){
   if(revision!==(configurations[id]?.revision || 0)) throw new Error("stale fixture revision");
   (window as any).__configCalls.push({id,action:"reset",revision});
   configurations[id]={revision:revision+1,value:""}; return this.integrationConfig(id);
 },
